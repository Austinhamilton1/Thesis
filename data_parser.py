...
import csv
from gexf import Node, Edge, Graph, Gexf
...
def generate_dynamic_graph(car_data: pd.DataFrame, threshold: float=15.0) -> dn.DynGraph:
    '''
        ...
        for j in range(len(positions)-1):
            ...
            for k in range(j+1, len(positions)):
                ...
                if distance <= threshold:
                    dynamic_graph.add_interaction(u=j, v=k, t=df['gpstime'].iloc[i])
    return dynamic_graph

def write_dynamic_graph(dynamic_graph: dn.DynGraph, filename: str) -> None:
    '''
    Writes a dynamic graph to a gexf file using helper functions 
    '''
    graph = Graph(mode='dynamic')
    for node in dynamic_graph.nodes(data=True):
        graph.add_node(node[0]) 
    for edge in dynamic_graph.interactions():
        for timestamp in edge[2]['t']:
            graph.add_edge(source=edge[0], target=edge[1], timestamp=timestamp[0])
    gexf = Gexf('Austin Hamilton', 'Dynamic VSN Data')
    gexf.load_graph(graph)
...
write_dynamic_graph(graph, 'graph.gexf')
